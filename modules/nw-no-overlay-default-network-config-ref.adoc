...
Specifies whether outbound pod traffic is SNATed to the node IP. Use `enabled` when pod IPs are not routable on the external network and `disabled` when they are routable.
