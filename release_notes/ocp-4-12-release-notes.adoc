...
* Previously, the reconciler would update a resource's annotation without making a copy of the resource. This caused an error that would terminate the the reconciler process. With this update, the reconciler no longer terminates due the error. (link:https://bugzilla.redhat.com/show_bug.cgi?id=2105045[*BZ#2105045*])
